...
API = "https://cn.bing.com/ttranslate"
INIT_URL = "https://cn.bing.com/translator"
USER_AGENT = "Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:59.0) Gecko/20100101 Firefox/59.0"

class BingHtmlTranslator:

  def __init__(self):
    self.s = requests.Session() # global session
    self.s.headers = {
      "User-Agent": USER_AGENT
    }
    self.s.get(INIT_URL)

  def translate(self, text, to='en', fr='ja', align=None):
    """Return translated text, which IS in unicode format
    ...
    try:
      ...
      dprint(r.text)
      t = json.loads(r.text)
      dprint(t["translationResponse"].upper())
      return t["translationResponse"].upper()
