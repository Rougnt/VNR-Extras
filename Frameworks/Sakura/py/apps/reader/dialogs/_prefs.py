...
class _TextTab(object):
  ...
  def load(self):
    ...
    for Name in ('Font', 'Shadow', 'Text', 'Subtitle', 'Comment', 'Danmaku',
                 'Romaji', 'Bing', 'Google', 'LecOnline', 'Infoseek', 'Babylon', 'Excite', 'Nifty', 'Systran', 'Transru', 'Naver', 'Baidu', 'Youdao', 'Tengxun', 'Caiyun', 'Moji', 'JBeijing', 'Fastait', 'Dreye', 'EzTrans', 'Transcat', 'Atlas', 'Lec', 'HanViet', 'VTrans'):
...
#    return ret
  ...
  def honyakuGroup(self):
    ...
    r += 1
    grid.addWidget(self.xuniButton, r, 1)
    ...
    r += 1
    grid.addWidget(self._createBrowseButton("https://api.fanyi.baidu.com/"), r, 0)
    row = self.baiduAPIButton(self.baiduButton)
    grid.addWidget(row, r, 1)
    r += 1
    grid.addLayout(self.baiduAPIEdittext, r, 1, 1, 2)

    if 'zh' not in blans:
      r += 1
      grid.addWidget(self._createBrowseButton("https://fanyi.qq.com"), r, 0)
      grid.addWidget(self.tengxunButton, r, 1)
      r += 1
      row = self.tengxunAPIButton(self.tengxunButton)
      grid.addWidget(row, r, 1)
      r += 1
      grid.addLayout(self.tengxunAPIEdittext, r, 1, 1, 2)
    ...
    if 'zh' not in blans:
      r += 1
      grid.addWidget(self._createBrowseButton("https://fanyi.caiyunapp.com"), r, 0)
      grid.addWidget(self.caiyunButton, r, 1)
      r += 1
      grid.addWidget(self._createBrowseButton("https://dashboard.caiyunapp.com/"), r, 0)
      row = self.caiyunAPIButton(self.caiyunButton)
      grid.addWidget(row, r, 1)
      r += 1
      grid.addLayout(self.caiyunTokenEdittext, r, 1, 1, 2)

    if 'zh' not in blans:
      r += 1
      grid.addWidget(self._createBrowseButton("https://www.mojidict.com"), r, 0)
      grid.addWidget(self.mojiButton, r, 1)
      r += 1
      grid.addLayout(self.mojiAPIEdit, r, 1, 1, 2)
  ...
  @memoizedproperty
  def xuniButton(self):
    ret = QtWidgets.QCheckBox(my.tr("使用JVM虚拟机替换python的百度腾讯有道彩云进行翻译"))
    ret.setStyleSheet("QCheckBox{color:blue}")
    ret.setChecked(settings.global_().isXuniEnabled())
    ret.toggled.connect(settings.global_().setXuniEnabled)
    return ret

  # @memoizedproperty
  def baiduAPIButton(self, btn):
    ret = QtWidgets.QCheckBox(my.tr("使用百度私人API替代网页翻译"))
    ret.setStyleSheet("QCheckBox{color:blue}")
    ret.setChecked(settings.global_().isBaiduAPIEnabled())
    ret.toggled.connect(settings.global_().setBaiduAPIEnabled)
    ret.setEnabled(btn.isChecked())
    btn.toggled.connect(ret.setEnabled)
    return ret

  @memoizedproperty
  def baiduAPIEdittext(self):
    ret = QtWidgets.QFormLayout()
    edit1 = QtWidgets.QLineEdit()
    edit1.setText(settings.global_().getBaiduAppID())
    edit1.textChanged.connect(settings.global_().setBaiduAppID)
    ret.addRow('appid', edit1)
    edit2 = QtWidgets.QLineEdit()
    edit2.setText(settings.global_().getBaiduKey())
    edit2.textChanged.connect(settings.global_().setBaiduKey)
    ret.addRow('key', edit2)
    return ret

  @memoizedproperty
  def tengxunButton(self):
    ret = QtWidgets.QCheckBox(my.tr("腾讯中文翻译服务"))
    ret.setStyleSheet("QCheckBox{color:blue}")
    ret.setChecked(settings.global_().isTengxunEnabled())
    ret.toggled.connect(settings.global_().setTengxunEnabled)
    return ret

  # @memoizedproperty
  def tengxunAPIButton(self, btn):
    ret = QtWidgets.QCheckBox(my.tr("使用腾讯私人API替代网页翻译"))
    ret.setStyleSheet("QCheckBox{color:blue}")
    ret.setChecked(settings.global_().isTengxunAPIEnabled())
    ret.toggled.connect(settings.global_().setTengxunAPIEnabled)
    ret.setEnabled(btn.isChecked())
    btn.toggled.connect(ret.setEnabled)
    return ret

  @memoizedproperty
  def tengxunAPIEdittext(self):
    ret = QtWidgets.QFormLayout()
    edit1 = QtWidgets.QLineEdit()
    edit1.setText(settings.global_().getTengxunAppID())
    edit1.textChanged.connect(settings.global_().setTengxunAppID)
    ret.addRow('secretId', edit1)
    edit2 = QtWidgets.QLineEdit()
    edit2.setText(settings.global_().getTengxunKey())
    edit2.textChanged.connect(settings.global_().setTengxunKey)
    ret.addRow('secretKey', edit2)
    return ret

  @memoizedproperty
  def caiyunButton(self):
    ret = QtWidgets.QCheckBox(my.tr("彩云中文翻译服务"))
    ret.setStyleSheet("QCheckBox{color:blue}")
    ret.setChecked(settings.global_().isCaiyunEnabled())
    ret.toggled.connect(settings.global_().setCaiyunEnabled)
    return ret

  def caiyunAPIButton(self, btn):
    ret = QtWidgets.QCheckBox(my.tr("使用自己的彩云Token，这可能提高速度与稳定性（默认是公共的），使用JVM将不会启用"))
    ret.setStyleSheet("QCheckBox{color:blue}")
    ret.setChecked(settings.global_().isCaiyunAPIEnabled())
    ret.toggled.connect(settings.global_().setCaiyunAPIEnabled)
    ret.setEnabled( btn.isChecked() and self.xuniButton.isChecked() )
    btn.toggled.connect(ret.setEnabled)  # auto switch
    return ret

  @memoizedproperty
  def caiyunTokenEdittext(self):
    ret = QtWidgets.QFormLayout()
    edit1 = QtWidgets.QLineEdit()
    edit1.setText(settings.global_().getCaiyunToken())
    edit1.textChanged.connect(settings.global_().setCaiyunToken)
    ret.addRow('token', edit1)
    return ret

  @memoizedproperty
  def mojiButton(self):
    ret = QtWidgets.QCheckBox(my.tr("Moji辞書翻訳"))
    ret.setStyleSheet("QCheckBox{color:blue}")
    ret.setChecked(settings.global_().isMojiAPIEnabled())
    ret.toggled.connect(settings.global_().setMojiAPIEnabled)
    return ret

  @memoizedproperty
  def mojiAPIEdit(self):
    ret = QtWidgets.QFormLayout()
    edit1 = QtWidgets.QLineEdit()
    edit1.setText(settings.global_().getMojiID())
    edit1.textChanged.connect(settings.global_().setMojiID)
    ret.addRow('Installtion Id', edit1)
    edit2 = QtWidgets.QLineEdit()
    edit2.setText(settings.global_().getMojiToken())
    edit2.textChanged.connect(settings.global_().setMojiToken)
    ret.addRow('token', edit2)
    return ret
...
class _EngineTab(object):
  ...
  def _getTranslators():
    ...
    try:
      ...
      if blans:
        if 'zh' in blans:
          for it in 'youdao', 'jbeijing', 'fastait', 'dreye', 'tengxun', 'caiyun':
            ret.remove(it)
        if 'ko' in blans:
