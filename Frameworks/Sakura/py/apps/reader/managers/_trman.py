...
class OnlineMachineTranslator(MachineTranslator):
  ...
  def _checkLanguages(self, to, fr):
    ...
    """
    # none type is not iterable error
...
class TengxunTranslator(OnlineMachineTranslator):
  key = 'tengxun' # override
  asyncSupported = False # override  disable async
  alignSupported = False # override  disable translation alignment

  def _checkLanguages(self, to, fr):
    """@reimp"""
    to, fr = super(TengxunTranslator, self)._checkLanguages(to=to, fr=fr)
    if to not in ('zhs', 'zht') and fr not in ('zhs', 'zht'):
      return None, None
    return to, fr

  def __init__(self, session=None, **kwargs):
    super(TengxunTranslator, self).__init__(**kwargs)

    from tengxun import tengxunfanyi
    tengxunfanyi.session = session or requests.Session()
    self.engine = tengxunfanyi

  __tengxun_repl_before = staticmethod(skstr.multireplacer({
    #u'【': u'‘', # open single quote
    #u'】': u'’：', # close single quote
    #u'「': u'“', # open double quote
    #u'」': u'”', # close double quote
  ...
  __tengxun_repl_after = staticmethod(skstr.multireplacer({
    #u'‘': u'【', # open single quote
    #u'’：': u'】', # close single quote
    #u'“': u'「', # open double quote
    #u'”': u'」', # close double quote
  ...
  def translate(self, text, to='zhs', fr='ja', async=False, emit=False, keepsNewLine=False, mark=None, context='', **kwargs):
    ...
    repl = self._encodeTranslation(text, to=to, fr=fr, emit=emit, context=context, proxies=proxies)
    if repl:
      repl = self.__tengxun_repl_before(repl)
      repl = self._translate(emit, repl,
          self.engine.translate,
          to, fr, async, keepsNewLine)
      if repl:
        ...
        if to == 'zht':
          repl = zhs2zht(repl)
        repl = self.__tengxun_repl_after(repl)
        repl = self._decodeTranslation(repl, to=to, fr=fr, mark=mark, emit=emit, context=context, proxies=proxies)
        self.cache.update(text, repl)
        if not keepsNewLine:
          repl = repl.strip()
...
class CaiyunTranslator(OnlineMachineTranslator):
  key = 'caiyun' # override
  asyncSupported = False # override  disable async
  alignSupported = False # override  disable translation alignment
  ...
  def _checkLanguages(self, to, fr):
    """@reimp"""
    to, fr = super(CaiyunTranslator, self)._checkLanguages(to=to, fr=fr)
    if to not in ('zhs', 'zht') and fr not in ('zhs', 'zht'):
      return None, None
    return to, fr

  def __init__(self, session=None, **kwargs):
    super(CaiyunTranslator, self).__init__(**kwargs)

    from caiyun import caiyunfanyi
    caiyunfanyi.session = session or requests.Session()
    self.engine = caiyunfanyi

  __caiyun_repl_before = staticmethod(skstr.multireplacer({
    #u'【': u'‘', # open single quote
    #u'】': u'’：', # close single quote
    #u'「': u'“', # open double quote
    #u'」': u'”', # close double quote
  ...
  __caiyun_repl_after = staticmethod(skstr.multireplacer({
    #u'‘': u'【', # open single quote
    #u'’：': u'】', # close single quote
    #u'“': u'「', # open double quote
    #u'”': u'」', # close double quote
  ...
  def translate(self, text, to='zhs', fr='ja', async=False, emit=False, keepsNewLine=False, mark=None, context='', **kwargs):
    ...
    repl = self._encodeTranslation(text, to=to, fr=fr, emit=emit, context=context, proxies=proxies)
    if repl:
      repl = self.__caiyun_repl_before(repl)
      repl = self._translate(emit, repl,
          self.engine.translate,
          to, fr, async, keepsNewLine)
      if repl:
        ...
        if to == 'zht':
          repl = zhs2zht(repl)
        repl = self.__caiyun_repl_after(repl)
        repl = self._decodeTranslation(repl, to=to, fr=fr, mark=mark, emit=emit, context=context, proxies=proxies)
        self.cache.update(text, repl)
        if not keepsNewLine:
          repl = repl.strip()
...
class MojiTranslator(OnlineMachineTranslator):
  key = 'moji' # override
  asyncSupported = False # override  disable async
  alignSupported = False # override  disable translation alignment
  ...
  def _checkLanguages(self, to, fr):
    """@reimp"""
    # what is to and fr
    # to, fr = super(MojiTranslator, self)._checkLanguages(to=to, fr=fr)
    to, fr = ('zhs', 'zhs')
    if to not in ('zhs', 'zht') and fr not in ('zhs', 'zht'):
      return None, None
    return to, fr
  ...
  def __init__(self, session=None, **kwargs):
    super(MojiTranslator, self).__init__(**kwargs)

    from moji import mojifanyi
    # I didn't see session in fanyi
    mojifanyi.session = session or requests.Session()
    self.engine = mojifanyi

  __moji_repl_before = staticmethod(skstr.multireplacer({
    #u'【': u'‘', # open single quote
    #u'】': u'’：', # close single quote
    #u'「': u'“', # open double quote
    #u'」': u'”', # close double quote
  ...
  __moji_repl_after = staticmethod(skstr.multireplacer({
    #u'‘': u'【', # open single quote
    #u'’：': u'】', # close single quote
    #u'“': u'「', # open double quote
    #u'”': u'」', # close double quote
  ...
  def translate(self, text, to='zhs', fr='ja', async=False, emit=False, keepsNewLine=False, mark=None, context='', **kwargs):
    ...
    repl = self._encodeTranslation(text, to=to, fr=fr, emit=emit, context=context, proxies=proxies)
    if repl:
      repl = self.__moji_repl_before(repl)
      repl = self._translate(emit, repl,
          self.engine.translate,
          to, fr, async, keepsNewLine)
      if repl:
        ...
        if to == 'zht':
          repl = zhs2zht(repl)
        repl = self.__moji_repl_after(repl)
        repl = self._decodeTranslation(repl, to=to, fr=fr, mark=mark, emit=emit, context=context, proxies=proxies)
        self.cache.update(text, repl)
        if not keepsNewLine:
          repl = repl.strip()
