...
from sakurakit import skclip, skdatetime, skevents, skpaths
...
class _TextManager(object):
  ...
  def __init__(self, q):
    ...
    self.contextsJson = None # dict, contexts
  ...
  # 存储语境
  def storeContext(self, hash, context):
    if not self.contextsJson:
      dm = dataman.manager()
      odct = OrderedDict()
      odct['gameName'] = dm.currentGameName()
      odct['gameId'] = dm.currentGameId()
      odct['gameMd5'] = dm.currentGameMd5()
      odct['contexts'] = []
      self.contextsJson = odct
      growl.msg('开始记录语境')
    entry = OrderedDict()
    entry['hash'] = str(hash)
    entry['context'] = context.split(defs.CONTEXT_SEP)
    entry['subtitle'] = ''
    self.contextsJson['contexts'].append(entry)

  # 保存语境
  def saveContexts(self):
    if self.contextsJson:
      dm = dataman.manager()
      path = 'contexts-%s-%d.json' % (dm.currentGameMd5(), skdatetime.current_unixtime())
      with open(path, 'wb') as file:
        file.write(json.dumps(self.contextsJson, ensure_ascii=False, indent=2))
        growl.msg('语境文件 %s 已保存' % path)
      self.contextsJson = None
  ...
  def showScenarioText(self, rawData=None, renderedData=None, text=None, agent=True):
    ...
    if cur_sz:
      ...
      # 导出语境
      if config.APP_DEBUG:
        self.storeContext(cur_hash, cur_ctx)
    else:
      cur_hash = 0
...
class TextManager(QObject):
  ...
  # MEGUMI 2019-09-09
  """
  def importSubtitlesFromFile(self):
    # Usable only in debug mode
    if not config.APP_DEBUG:
      return
    path, filter = QtWidgets.QFileDialog.getOpenFileName(None, '请选择编辑好的JSON文件', skpaths.DESKTOP, 'JSON file (*.json)')
    if not path:
      return
    with open(path, 'rb') as f:
      ...
      md5 = js['gameMd5']
      ctx = js['contexts']
      # Get access to the data manager
      dm = dataman.manager()
      d = dm._DataManager__d
      # Check if the file matches the current game
      if gid != dm.currentGameId() or md5 != dm.currentGameMd5():
        dwarn('This file does not match current game')
        growl.warn('所选文件与当前运行的游戏不匹配')
        return
      # Check if the user is logged in
      if not d.user.isLoginable():
        dwarn("not login")
        growl.warn('登录后才能导入字幕')
        return
      # Iterate all contexts
      for c in ctx:
        self._importSubtitlesData(c)
      # Start submitting all comments now
      num = len(d.comments)
      if num:
        dprint('Added %d subtitls' % num)
        growl.msg('已导入 %d 个字幕，稍后自动提交。' % num)
        # submit
  ...
  # MEGUMI 2019-09-09
  """
  def _importSubtitlesData(self, ctx):
    # Get data from JSON object
    hash = ctx['hash']
    text = ctx['subtitle']
    ctx2 = ctx['context']
    # Check empty subtitle
    if not text:
      dwarn('Blank subtitle ignored!!! hash = %s' % hash)
      return
    # Get access to the data manager
    dm = dataman.manager()
    d = dm._DataManager__d
    ...
    cm = dataman.Comment(
      text        = text,
      type        = 'subtitle',
      hash        = numpy.int64(hash),
      context     = defs.CONTEXT_SEP.join(ctx2),
      contextSize = len(ctx2),
      gameId      = dm.currentGameId(),
      gameMd5     = dm.currentGameMd5(),
      userId      = dm.user().id,
      language    = 'zhs',
      timestamp   = skdatetime.current_unixtime())
    # Reset ID for new comment
    cm.id = 0
    # Update current context
    dm.updateContext(cm.d.hash, cm.d.context)
    # Update current context
    if cm.context:
      d.contexts[cm.hash] = cm.context
    # Add new comment to data manager
    if cm.hash in d.comments:
      d.comments[cm.hash].append(cm)
    else:
      d.comments[cm.hash] = [cm]
    # Add the comment to dirty list, we will submit later
    d.dirtyComments.add(cm)
    # Update GUI (Dialog)
    # dm.commentAdded.emit(cm)
    # Output to debugger
    dprint('Added new subtitle hash = %s' % hash)
...
class TextManagerProxy(QObject):
  ...
  """
  @Slot()
  def importSubtitlesFromFile(self): manager().importSubtitlesFromFile()
