...
class _TranslatorManager(object):
  ...
  @memoizedproperty
  def tengxunTranslator(self):
    return self._newtr(_trman.TengxunTranslator(
      abortSignal=self.abortSignal,
      session=self.session,
      postprocess=self.postprocessCharacterset))

  @memoizedproperty
  def caiyunTranslator(self):
    return self._newtr(_trman.CaiyunTranslator(
      abortSignal=self.abortSignal,
      session=self.session,
      postprocess=self.postprocessCharacterset))

  @memoizedproperty
  def mojiTranslator(self):
    return self._newtr(_trman.MojiTranslator(
      abortSignal=self.abortSignal,
      session=self.session,
      postprocess=self.postprocessCharacterset))
  ...
  def iterOnlineTranslators(self, reverse=False):
    ...
    if self.online:
      if reverse:
        ...
        if self.tengxunEnabled: yield self.tengxunTranslator
        if self.caiyunEnabled: yield self.caiyunTranslator
        if self.baiduEnabled: yield self.baiduTranslator
        if self.mojiEnabled: yield self.mojiTranslator
      else:
        if self.mojiEnabled: yield self.mojiTranslator
        if self.baiduEnabled: yield self.baiduTranslator
        if self.youdaoEnabled: yield self.youdaoTranslator
...
class TranslatorManager(QObject):
  ...
  def isTengxunEnabled(self): return self.__d.tengxunEnabled
  def setTengxunEnabled(self, value): self.__d.tengxunEnabled = value

  def isCaiyunEnabled(self): return self.__d.caiyunEnabled
  def setCaiyunEnabled(self, value): self.__d.caiyunEnabled = value

  def isMojiEnabled(self): return self.__d.mojiEnabled
  def setMojiEnabled(self, value): self.__d.mojiEnabled = value

  def isXuniEnabled(self): return self.__d.xuniEnabled
  def setXuniEnabled(self, value): self.__d.xuniEnabled = value
  ...
  def enabledEngines(self, fr=''): # str -> [str]
    ...
    if d.tengxunEnabled: r.append('tengxun')
    if d.caiyunEnabled: r.append('caiyun')
    if d.mojiEnabled: r.append('moji')
  ...
  def getTranslationTargetLanguages(self): # -> [str]
    ...
    if d.jbeijingEnabled or d.fastaitEnabled or d.youdaoEnabled or d.tengxunEnabled or d.caiyunEnabled: #or d.vtransEnabled
  ...
  def translateApply(self, func, text, fr='ja', to='', keepsNewLine=None, mark=None, scriptEnabled=None, ehndEnabled=None, rubyEnabled=None, context='', **kwargs):
    ...
    if settings.global_().isXuniEnabled():
      try:
        start = time.time()
        b1 = b2 = b3 = b4 = False
        for it in translators:
          b1 = b1 | (it.key == 'baidu')
          b2 = b2 | (it.key == 'tengxun')
          b3 = b3 | (it.key == 'youdao')
          b4 = b4 | (it.key == 'caiyun')
        ...
        m = re.match(u"(「|…|？|！|~|!|\?|\$|#|（|」|）)+", text)
        if m is not None:
          if m.end() - m.start() == len(text):
            tran = [text, text, text, text]
          else:
            n = re.search(u"(。|！|？|…|）|」)+$", text[m.end():])
            if n is not None:
              p = m.end()
              q = n.start()
            tran = fanyi.translate(text[p:q + p], b1, b2, b3, b4)
        else:
          tran = fanyi.translate(text, b1, b2, b3, b4)
        if b1: func(text[0:p] + tran[0] + text[q + p:r], u'zhs', 'baidu', None, **kwargs)
        if b2: func(text[0:p] + tran[1] + text[q + p:r], u'zhs', 'tengxun', None, **kwargs)
        if b3: func(text[0:p] + tran[2] + text[q + p:r], u'zhs', 'youdao', None, **kwargs)
        if b4: func(text[0:p] + tran[3] + text[q + p:r], u'zhs', 'caiyun', None, **kwargs)
        end = time.time()
        dprint("fanyi time:" + str(end - start))
      except Exception, e:
        dwarn(e)
    else:
      for it in translators:
